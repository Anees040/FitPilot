...
Always read this file, then `mdFiles/STATUS.md` (current state + current task).
Read other docs ONLY when relevant to the task:
- `mdFiles/ARCHITECTURE.md` — before creating/moving any file or class
- `mdFiles/DESIGN_SYSTEM.md` — before writing any UI
- `mdFiles/DOMAIN_RULES.md` — before writing any calorie/streak/burn logic
- `mdFiles/DECISIONS.md` — only if you think a past decision blocks you
...
2. Layered architecture (mdFiles/ARCHITECTURE.md). `lib/domain/` is pure Dart:
...
4. UI uses ONLY the tokens in mdFiles/DESIGN_SYSTEM.md. No blue, no gradients,
...
3. Update `mdFiles/STATUS.md` (move task to Done, set Next, note any issues)
4. Append one line to `mdFiles/DECISIONS.md` ONLY if a real design decision was made
